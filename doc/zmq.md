...
The Multicoin Core daemon can be configured to act as a trusted "border
router", implementing the multicoin wire protocol and relay, making
...
The ZeroMQ feature in Multicoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building multicoind:
...
For instance:
    ...
    $ multicoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/multicoind.tx.raw \
...
These options can also be provided in multicoin.conf.
...
From the perspective of multicoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into multicoind directly. Furthermore, no information is
...
using. Multicoind appends an up-counting sequence number to each
