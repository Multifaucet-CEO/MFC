    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional MUC outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes MUC and MWEB fees)
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // If MUC-to-MUC fails, create a peg-out transaction
